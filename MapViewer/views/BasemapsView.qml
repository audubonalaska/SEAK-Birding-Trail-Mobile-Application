    ...
    delegate: Pane {
        ...
        contentItem: Item{
            ColumnLayout {
                ...
                // Basemaps Title text is translated if it is present in the Translator mapping dictionary, else, the input string from model is displayed as-is
                Controls.BaseText {
                    text: ( title > "" && basemapsTranslator.basemapTranslatorDict.hasOwnProperty(title) )
                          ? basemapsTranslator.basemapTranslatorDict[title]
                          : title
                    maximumLineCount: 2
